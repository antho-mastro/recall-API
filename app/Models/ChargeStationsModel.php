...
class ChargeStationsModel extends BaseModel
{
    private string $table_name = 'charge_stations';
    ...
    public function getAllStations(){
        $sql = "SELECT * FROM charge_stations";
        return $this->paginate($sql);
    }

    public function getStationById(int $station_id)
    {
        $sql = "SELECT * FROM $this->table_name WHERE StationID = :StationID";
        return $this->run($sql, [":StationID" => $station_id])->fetchAll();
    }

    public function stationsCreate(array $new_stations)
    {
        return $this->insert($this->table_name, $new_stations);
    }

    public function deleteStation($station_id)
    {
        $this->delete($this->table_name, ["StationID" => $station_id]);
    }


    public function StationUpdate(array $station_data, array $station_id)
    {
        $this->update($this->table_name, $station_data, $station_id);
    }
