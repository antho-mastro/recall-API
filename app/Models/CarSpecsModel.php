...
class CarSpecsModel extends BaseModel
{
    private string $table_name = 'car_specs';
    ...
    public function getAllSpecs(){
        $sql = "SELECT * FROM car_specs";
        return $this->paginate($sql);
    }

    public function getSpecById(int $specs_id)
    {
        $sql = "SELECT * FROM $this->table_name WHERE SpecsID = :SpecsID";
        return $this->run($sql, [":SpecsID" => $specs_id])->fetchAll();
    }

    public function specsCreate(array $new_specs)
    {
        return $this->insert($this->table_name, $new_specs);
    }

    public function deletespecs($specs_id)
    {
        $this->delete($this->table_name, ["SpecsID" => $specs_id]);
    }


    public function SpecsUpdate(array $specs_data, array $specs_id)
    {
        $this->update($this->table_name, $specs_data, $specs_id);
    }
